...
i3ds::MessageRecord::store(std::ofstream &output_file)
{
  ...
  output_file.write((char*)&address.node, sizeof(i3ds_asn1::NodeID));
  output_file.write((char*)&address.endpoint, sizeof(i3ds_asn1::EndpointID));
...
i3ds::MessageRecord::load(std::ifstream &input_file)
{
  ...
  input_file.read((char*)&node, sizeof(i3ds_asn1::NodeID));
  input_file.read((char*)&endpoint, sizeof(i3ds_asn1::EndpointID));
  ...
  for (uint32_t i = 0; i < payloads; ++i)
    {
      ...
      i3ds_asn1::byte* buffer = (i3ds_asn1::byte*)malloc(msg_size);
...
i3ds::SessionRecording::store_header()
{
  ...
  for (i3ds_asn1::NodeID nid : node_ids)
    {
      hdr_file.write((char*)&nid, sizeof(i3ds_asn1::NodeID));
    }
  hdr_file.write((char*)&endpoint_id_set, sizeof(bool));
  hdr_file.write((char*)&endpoint_id, sizeof(i3ds_asn1::EndpointID));
...
i3ds::SessionReader::SessionReader(std::string filename) : input_file(filename, std::ios::binary)
{
  ...
  for (size_t i=0; i< node_id_count; i++)
    {
      hdr_file.read((char*)&node_ids[i], sizeof(i3ds_asn1::NodeID));
    }
  hdr_file.read((char*)&endpoint_id_set, sizeof(bool));
  hdr_file.read((char*)&endpoint_id, sizeof(i3ds_asn1::EndpointID));
