...
i3ds::CameraMerger::publisher_thread_func()
{
  ...
  while (run_publiser_thread_)
  {
    ...
    BOOST_LOG_TRIVIAL(trace) << "CameraMerger received frames with timestamps " << cam_1_data.timestamp_ << " and " << cam_2_data.timestamp_;
    auto t_diff = static_cast<unsigned int>(std::abs(static_cast<long>(cam_1_data.timestamp_) - static_cast<long>(cam_2_data.timestamp_)));
